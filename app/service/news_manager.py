...
class NewsManager:
    ...
    def get_news_by_id(self, news_id: str) -> NewsItem:
        ...
        if not news:
            ...
            raise HTTPException(
                status_code=404, detail=f"News item with ID {news_id} not found"
            )
    ...
    def list_news(self, req: NewsListReq) -> NewsListResp:
        ...
        if req.category:
            ...
            filters.append(
                News.title.contains(req.category) | News.summary.contains(req.category)
            )
        ...
        return NewsListResp(total_count=total_count, items=pydantic_items)
