...
from app.schema import NewsItem, ReportGenerateReq, ReportGenerateResp
...
class InfoInterpreter:
    ...
    def __init__(self, settings: Setting = Depends(get_setting)):
        ...
        self._system_prompt_path = self._settings.project_root / "app" / "prompts" / "report_prompt.txt"
        
    def _setup_gemini_client(self):
        """Set up the Gemini API client with the API key from settings."""
        if not self._settings.gemini_api_key:
            logger.warning("Gemini API key not found in settings. Report generation may not work.")
    ...
    def _load_system_prompt(self) -> str:
        """Load the system prompt for report generation from file."""
        try:
            if not self._system_prompt_path.exists():
                logger.warning(f"System prompt file not found at {self._system_prompt_path}")
                return ""
    ...
    async def generate_report(self, req: ReportGenerateReq) -> ReportGenerateResp:
        """Generate a report from news data using Gemini API."""
        start_time = time.time()
        logger.info(f"Starting report generation with parameters: {req.model_dump()}")
        ...
        try:
            ...
            if not req.filename and req.article_ids:
                ...
                if not all_files:
                    raise HTTPException(status_code=404, detail="No JSON files found in assets directory")
                file_path = all_files[0]
                logger.info(f"No filename provided, using latest file: {file_path.name}")
            elif req.filename:
                file_path = self._settings.assets_dir / req.filename
            ...
            logger.info(f"Using {len(filtered_items)} items for report generation after filtering")
            ...
            context_text += "Additional Instructions: Please format the output exactly as follows:\n"
            context_text += "1. Begin with a title in the format: '🌐 AI 每日速递 |📅 人工智能期刊" + current_date + " | 汇总 | 最新人工智能动态 🚀(预计阅读时间:20~25分钟)：'\n"
            ...
            context_text += "   - Title with emoji (e.g., '💼 Luminance获得7500万美元融资')\n"
    ...
    def _extract_sections(self, report_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if toc_index > 0:
                toc_content = []
                i = toc_index + 1
                while i < len(lines) and not (lines[i].strip().startswith("1.") or "摘要:" in lines[i]):
                    if lines[i].strip():
            ...
            for line in lines:
                if line.strip() and any(emoji in line[:3] for emoji in ["🚀", "💰", "🤖", "📱", "💼", "📊", "🔍", "🌐", "💻", "🎯", "📈"]):
                    if "摘要:" not in line and "关键点:" not in line and len(line.strip()) < 50:
