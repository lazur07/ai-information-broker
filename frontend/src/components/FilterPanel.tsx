...
import { ScrapeReq, NewsSource } from "../types";

interface FilterPanelProps {
  onFilter: (params: ScrapeReq) => void;
  loading: boolean;
}

const FilterPanel: React.FC<FilterPanelProps> = ({ onFilter, loading }) => {
  const [filters, setFilters] = useState<ScrapeReq>({
    days_back: 1,
    category: "AI",
  ...
  return (
    <div className="bg-white rounded-ios shadow-ios p-5 mb-5">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="pt-2">
          <span className="block text-sm font-semibold text-ios-gray-800 mb-2">
            Sources
          </span>
        ...
        <div>
          ...
          <input
            ...
            value={filters.limit}
            onChange={(e) =>
              setFilters({ ...filters, limit: parseInt(e.target.value) || 20 })
            }
            className="block w-full rounded-ios bg-ios-gray-100 border-none px-4 py-3 text-ios-gray-800 focus:ring-2 focus:ring-ios-blue focus:bg-white focus:outline-none transition duration-200"
          />
        </div>
